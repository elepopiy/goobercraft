...
class World {
    ...
    columns = new Map();
    static columnKey(chunkX, chunkZ) {
        return `${chunkX},${chunkZ}`;
    }
    // ── Chunk Yönetimi ────────────────────────────────────────────────────
    setColumn(chunkX, chunkZ, column) {
    ...
    getBlock(position) {
        ...
        try {
            ...
            return column.getBlock(new vec3_1.Vec3(localX, y, localZ)) ?? null;
        }
        catch {
            return null;
        }
     ...
     */
    setBlockStateId(position, stateId) {
        const chunkX = Math.floor(position.x) >> 4;
        const chunkZ = Math.floor(position.z) >> 4;
        const column = this.getColumn(chunkX, chunkZ);
        if (!column)
        ...
        try {
            ...
            if (typeof column.setBlockStateId === "function") {
                column.setBlockStateId(new vec3_1.Vec3(localX, y, localZ), stateId);
            }
            else if (typeof column.setBlock === "function") {
                column.setBlock(new vec3_1.Vec3(localX, y, localZ), { stateId });
            }
        }
        catch {
    ...
    getBiome(position) {
        ...
        try {
            ...
            if (typeof column.getBiome === "function") {
                return column.getBiome(new vec3_1.Vec3(localX, y, localZ)) ?? null;
            }
        }
        catch {
            // Yoksay
     ...
     */
    canSeeBlock(eyePos, blockPos, maxDistance = 4.5) {
    ...
    findBlocks(options) {
        ...
        const point = (options.point ?? new vec3_1.Vec3(0, 0, 0)).floored();
        ...
        const start = new vec3_1.Vec3(Math.floor(point.x / 16), Math.floor(point.y / 16), Math.floor(point.z / 16));
        ...
        while (next) {
            ...
            if (column && !visitedSections.has(next.toString())) {
                const begin = new vec3_1.Vec3(next.x * 16, sectionY * 16, next.z * 16);
