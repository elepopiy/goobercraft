...
class GoalLookAtBlock extends Goal {
    ...
    isEnd(node) {
        if (node.distanceTo(this.pos.offset(0, this.entityHeight, 0)) > this.reach)
            return false;
...
class GoalPlaceBlock extends Goal {
    constructor(pos, world, options) {
        ...
        if (!this.options.range)
            this.options.range = 5;
        if (!('LOS' in this.options))
            this.options.LOS = true;
    ...
    getFaceAndRef(headPos) {
        for (const [face, to, ref] of this.facesPos) {
            ...
            if (dir.norm() > this.options.range)
                continue;
            if (!this.checkFacing(dir))
                continue;
            if (!this.options.LOS) {
                return { face, to, ref };
            }
