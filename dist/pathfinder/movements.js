...
class Movements {
    ...
    exclusionPlace(block) {
        if (this.exclusionAreasPlace.length === 0)
            return 0;
        let weight = 0;
    ...
    exclusionStep(block) {
        if (this.exclusionAreasStep.length === 0)
            return 0;
        let weight = 0;
    ...
    exclusionBreak(block) {
        if (this.exclusionAreasBreak.length === 0)
            return 0;
        let weight = 0;
    ...
    getNumEntitiesAt(pos, dx, dy, dz) {
        if (this.allowEntityDetection === false)
            return 0;
        if (!pos)
            return 0;
        const y = pos.y + dy;
    ...
    safeToBreak(block) {
        ...
        if (this.dontCreateFlow) {
            ...
            if (this.getBlock(block.position, 0, 1, 0).liquid)
                return false;
            if (this.getBlock(block.position, -1, 0, 0).liquid)
                return false;
            if (this.getBlock(block.position, 1, 0, 0).liquid)
                return false;
            if (this.getBlock(block.position, 0, 0, -1).liquid)
                return false;
            if (this.getBlock(block.position, 0, 0, 1).liquid)
                return false;
        }
    ...
    safeOrBreak(block, toBreak) {
        ...
        if (!this.safeToBreak(block))
            return 100; // Can't break, so can't move
    ...
    getMoveJumpUp(node, dir, neighbors) {
        ...
        if (blockH.physical && (this.getNumEntitiesAt(blockH.position, 0, 1, 0) > 0))
            return;
        if (blockB.physical && !blockH.physical && !blockC.physical && (this.getNumEntitiesAt(blockB.position, 0, 1, 0) > 0))
            return; // It is fine if an ent falls on B so long as we don't need to replace block C
        if (!blockC.physical) {
            if (node.remainingBlocks === 0)
                return; // not enough blocks to place
            if (this.getNumEntitiesAt(blockC.position, 0, 0, 0) > 0)
                return; // Check for any entities in the way of a block placement
            const blockD = this.getBlock(node, dir.x, -1, dir.z);
            if (!blockD.physical) {
                if (node.remainingBlocks === 1)
                    return; // not enough blocks to place
                if (this.getNumEntitiesAt(blockD.position, 0, 0, 0) > 0)
                    return; // Check for any entities in the way of a block placement
    ...
    getMoveForward(node, dir, neighbors) {
        ...
        if (!blockD.physical && !blockC.liquid) {
            if (node.remainingBlocks === 0)
                return; // not enough blocks to place
            if (this.getNumEntitiesAt(blockD.position, 0, 0, 0) > 0)
                return; // D intersects an entity hitbox
            if (!blockD.replaceable) {
                if (!this.safeToBreak(blockD))
                    return;
                cost += this.exclusionBreak(blockD);
    ...
    getMoveDropDown(node, dir, neighbors) {
        ...
        if (!blockLand)
            return;
        if (!this.infiniteLiquidDropdownDistance && ((node.y - blockLand.position.y) > this.maxDropDown))
    ...
    getMoveUp(node, neighbors) {
        ...
        if (block1.liquid)
            return;
        if (this.getNumEntitiesAt(node, 0, 0, 0) > 0)
            return; // an entity (besides the player) is blocking the building area
        ...
        if (!block1.climbable) {
            ...
            if (block0.physical && block0.height - node.y < -0.2)
                return; // cannot jump-place from a half block
